...
class PolyphemusTrainer():
    ...
    def train(self, trainloader, validloader=None, epochs=5, early_exit=None):
    ...
    def _pitch_accuracy(self, c_logits, c_tensor, drums=None, is_drum=None):
        ...
        not_pad = (pitch_true != PitchToken.PAD_M.value)
    ...
    def _note_accuracy(self, c_logits, c_tensor):
        ...
        not_pad_p = (pitch_true != PitchToken.PAD_M.value)
        ...
        note_accuracy = torch.sum(
            torch.logical_and(correct_p, correct_d)) / torch.sum(correct_d)
